...
class ReadingPlanRepositoryImpl implements ReadingPlanRepository {
  ...
  /// Helper method to save plan to local and conditionally to remote
  Future<void> _savePlan(ReadingPlanModel plan) async {
    ...
    // Only sync to remote if not a local-only user
    if (plan.userId != 'local_user') {
  ...
  Future<Either<Failure, ReadingPlanEntity>> getReadingPlan(String userId) async {
    try {
      ...
      // For local-only users, don't try to fetch from remote
      if (userId == 'local_user') {
        return Left(CacheFailure('No local reading plan found'));
      }
  ...
  Future<void> _syncInBackground(String userId) async {
    try {
      // Skip sync for local-only users
    ...
    try {
      ...
      if (markCompleted) {
        switch (currentReadingMode) {
          case 'standard':
            // Standard mode: increment the specific list's progress
            if (isMcheyneList) {
              updatedMcheyneProgress[listId] = chapter + 1;
            } else {
              updatedProgress[listId] = chapter + 1;
            }
            break;
          case 'index':
            // Index mode: don't increment individual progress
            // Will check if all lists complete below to increment indexProgress
            break;
          case 'calendar':
      ...
      if (markCompleted) {
        ...
        if (isMcheyneList) {
          ...
          // Index mode: check if all McCheyne lists are completed
          if (currentReadingMode == 'index') {
            final allCompleted = updatedMcheyneListCompletionStatus.length == 4 &&
                updatedMcheyneListCompletionStatus.values.every((completed) => completed == true);
            if (allCompleted) {
              newMcheyneIndexProgress = plan.mcheyneIndexProgress + 1;
            }
          }
        } else {
          ...
          // Index mode: check if all Horner lists are completed
          if (currentReadingMode == 'index') {
            final allCompleted = updatedListCompletionStatus.length == 10 &&
                updatedListCompletionStatus.values.every((completed) => completed == true);
            if (allCompleted) {
              newIndexProgress = plan.indexProgress + 1;
            }
          }
        }
  ...
  Future<Either<Failure, bool>> performDailyResetIfNeeded(String userId) async {
    try {
      ...
      final now = DateTime.now();
      final lastUpdate = plan.lastUpdated;

      // Check if it's a new day based on last update
  ...
  Future<Either<Failure, void>> setIndexProgress(String userId, int indexProgress) async {
    try {
      ...
      final updatedPlan = ReadingPlanModel(
        ...
        readingMode: plan.readingMode,
        progress: plan.progress,
        indexProgress: indexProgress, // Update this field
        lastUpdated: DateTime.now(),
        completedToday: plan.completedToday,
        listCompletionStatus: plan.listCompletionStatus,
        currentStreak: plan.currentStreak,
  ...
  Future<Either<Failure, void>> setMcheyneIndexProgress(String userId, int indexProgress) async {
    try {
      ...
      // McCheyne has 336 readings total - wrap around if exceeds
      const int mcheyneTotal = 336;
      int wrappedIndex = indexProgress;
      if (indexProgress > mcheyneTotal) {
        // Wrap around: 337 -> 1, 338 -> 2, etc.
