...
class GetTodaysReadings {
  ...
  Future<Either<Failure, TodaysReadingsResult>> call(String userId) async {
    ...
    return planResult.fold(
      ...
      (plan) {
        ...
        final isIndex = plan.readingMode == 'index' || plan.mcheyneReadingMode == 'index';
        final indexProgress = plan.planSystem == 'horner' ? plan.indexProgress : plan.mcheyneIndexProgress;
        if (plan.planSystem == 'horner') {
          ...
          for (var listId in orderedListIds) {
            ...
            switch (plan.readingMode) {
              case 'index':
                // In index mode, wrap the shared indexProgress to each list's total
                displayChapter = ((plan.indexProgress - 1) % totalChapters) + 1;
                break;
              case 'calendar':
          ...
          for (var listId in mcheyneListIds) {
            ...
            switch (plan.mcheyneReadingMode) {
              case 'index':
                // In index mode, wrap the shared mcheyneIndexProgress to total readings (365)
                displayIndex = ((plan.mcheyneIndexProgress - 1) % totalReadings) + 1;
                break;
              case 'calendar':
                final now = DateTime.now();
                final dayOfYear = now.difference(DateTime(now.year, 1, 1)).inDays + 1;
        ...
        return Right(TodaysReadingsResult(
          readings: readings,
          isIndex: isIndex,
